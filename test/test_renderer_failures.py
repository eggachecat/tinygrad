...
def _test_uop_result(inputs: List[Tensor], stores: List[UOp], local_size=None):
    ...
    outbufs = [
        Buffer(
            ...
            (dtype := u.src[1].dtype),
            initial_value=np.zeros(sz, dtype=_to_np_dtype(dtype)).data,
        )
        for u in uops
    ...
    ei = CompiledRunner(
        ProgramSpec(
            "test", src, Device.DEFAULT, uops[-1], uops=uops, local_size=local_size
        )
    )
...
@unittest.skipIf(
    not isinstance(Device[Device.DEFAULT].renderer, CStyleLanguage),
    "uops are for cstyle",
)
...
@unittest.skipIf(
    not isinstance(Device[Device.DEFAULT].renderer, PTXRenderer),
...
class TestPTXFailures(unittest.TestCase):
    def test_gated_store_with_alu(self):
        ...
        gated_alu_store = UOp(
            Ops.STORE, dtypes.void, (a.index(lidx0, gate_alu), UOp.const(dtypes.int, 1))
        )
