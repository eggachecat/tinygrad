...
def compare_weights_both(url):
    ...
    torch_weights = get_state_dict(
        torch.load(fn, map_location=torch.device("cpu"), weights_only=False),
    ...
    for k in tg_weights:
        ...
        if torch_weights[k].requires_grad:
            torch_weights[k] = torch_weights[k].detach()
...
class TestRawDiskBuffer(unittest.TestCase):
    @unittest.skipIf(
        not test_fn.exists(), "download LLaMA weights for read in speed tests"
    )
    ...
    def test_bitcasts_on_disk(self):
        ...
        t.bitcast(dtypes.uint16).assign(
            Tensor.full((128, 64), 0x4248, dtype=dtypes.uint16)
        ).realize()
        ...
        t.bitcast(dtypes.float32).assign(
            Tensor.full((128, 32), 3.1415927, dtype=dtypes.float32)
        ).realize()
        _test_bitcasted(t, dtypes.float32, 3.1415927)
        _test_bitcasted(t, dtypes.uint32, 0x40490FDB)
        # doesn't suport normal cast
        with self.assertRaises(NotImplementedError):
            Tensor.empty((4,), dtype=dtypes.int16, device=f"disk:{tmp}").cast(
                dtypes.float16
            ).realize()
        ...
        with self.assertRaises(RuntimeError):
            ...
            Tensor.empty((3,), dtype=dtypes.int8, device=f"DISK:{tmp}").bitcast(
                dtypes.float16
            )
        ...
        with self.assertRaises(RuntimeError):
            ...
            Tensor.empty(
                (4,), dtype=dtypes.int8, requires_grad=True, device=f"DISK:{tmp}"
...
class TestSafetensors(unittest.TestCase):
    ...
    def test_save_all_dtypes(self):
        for dtype in dtypes.fields().values():
            ...
            if dtype in [dtypes.double] and Device.DEFAULT == "METAL":
                continue  # not supported on METAL
            path = temp(f"ones.{dtype}.safetensors")
            ones = Tensor(np.random.rand(10, 10), dtype=dtype)
            safe_save(get_state_dict(ones), path)
            np.testing.assert_equal(
                ones.numpy(), list(safe_load(path).values())[0].numpy()
            )
...
class TestDiskTensor(unittest.TestCase):
    ...
    def test_simple_read(self):
        ...
        t = Tensor.empty(
            16, 16, device=f"disk:{temp('dt_simple_read')}", dtype=dtypes.uint8
        )
    ...
    def test_simple_read_bitcast(self):
        ...
        t = Tensor.empty(
            16,
            16 * 2,
            device=f"disk:{temp('dt_simple_read_bitcast')}",
            dtype=dtypes.uint8,
        )
    ...
    def test_simple_read_bitcast_alt(self):
        ...
        t = Tensor.empty(
            16,
            16 * 2,
            device=f"disk:{temp('dt_simple_read_bitcast_alt')}",
            dtype=dtypes.uint8,
        )
    ...
    def test_assign_slice(self):
        ...
        helper_test_disk_tensor(
            "dt_assign_slice_1",
            [0, 1, 2, 3],
            lambda x: assign(x, slice(0, 2), [13, 12]),
        )
        helper_test_disk_tensor(
            "dt_assign_slice_2",
            [[0, 1, 2, 3], [4, 5, 6, 7]],
    ...
    def test_reshape(self):
        helper_test_disk_tensor(
            "dt_reshape_1", [1, 2, 3, 4, 5], lambda x: x.reshape((1, 5))
        )
        helper_test_disk_tensor(
            "dt_reshape_2", [1, 2, 3, 4], lambda x: x.reshape((2, 2))
        )
    ...
    def test_assign_to_different_dtype(self):
        ...
        t = Tensor.empty(
            10,
            device=f'disk:{temp("dt_assign_to_different_dtype")}',
            dtype=dtypes.int64,
        )
    ...
    def test_bitcast(self):
        with open(temp("dt_bitcast"), "wb") as f:
            f.write(bytes(range(10, 20)))
    ...
    def test_bitcast_view(self):
        with open(temp("dt_bitcast_view"), "wb") as f:
            f.write(bytes(range(10, 24)))
        t = Tensor.empty(
            3, dtype=dtypes.uint, device=f"disk:{temp('dt_bitcast_view')}"
        ).shrink([(0, 2)])
    ...
    def test_bf16_disk_write_read(self):
        ...
        with open(temp("dt_bf16_disk_write_read_f32"), "rb") as f:
            dat = f.read()
        adat = b"".join([dat[i + 2 : i + 4] for i in range(0, len(dat), 4)])
        with open(temp("dt_bf16_disk_write_read_bf16"), "wb") as f:
            f.write(adat)

        t = Tensor.empty(
            5,
            dtype=dtypes.bfloat16,
            device=f"disk:{temp('dt_bf16_disk_write_read_bf16')}",
        )
        ct = t.llvm_bf16_cast(dtypes.float)
        assert ct.numpy().tolist() == [9984.0, -1, -1000, -9984, 20]
    ...
    def test_copy_from_disk(self):
        ...
        t = Tensor.empty(
            256 * 1024, device=f"disk:{temp('dt_copy_from_disk')}", dtype=dtypes.uint8
        )
    ...
    def test_copy_from_disk_offset(self):
        ...
        for off in [314, 991, 2048, 4096]:
            t = Tensor.empty(
                256 * 1024,
                device=f"disk:{temp('dt_copy_from_disk_offset')}",
                dtype=dtypes.uint8,
            )[off:]
            on_dev = t.to(Device.DEFAULT).realize()
            np.testing.assert_equal(on_dev.numpy(), t.numpy())
    ...
    def test_copy_from_disk_huge(self):
        if CI and not hasattr(Device["DISK"], "io_uring"):
            self.skipTest("slow on ci without iouring")
        ...
        for off in [0, 551]:
            t = Tensor.empty(
                256 * 1024 * 256,
                device=f"disk:{temp('dt_copy_from_disk_huge')}",
                dtype=dtypes.uint8,
            )[off:]
            on_dev = t.to(Device.DEFAULT).realize()
            np.testing.assert_equal(on_dev.numpy(), t.numpy())
...
class TestPathTensor(unittest.TestCase):
    ...
    def test_path_tensor_no_device(self):
        ...
        np.testing.assert_array_equal(
            t.numpy(), np.frombuffer(self.test_data, dtype=np.uint8)
        )
    ...
    def test_path_tensor_with_dtype(self):
        ...
        self.assertTrue(t.device.startswith("DISK:"))
        np.testing.assert_array_equal(
            t.numpy(), np.frombuffer(self.test_data, dtype=np.int16)
        )
    ...
    def test_path_tensor_copy_to_device(self):
        ...
        np.testing.assert_array_equal(
            t_cpu.numpy(), np.frombuffer(self.test_data, dtype=np.uint8)
        )
