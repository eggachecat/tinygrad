...
class CheckingShapeTracker:
    ...
    def assert_same(self):
        x = [
            (v[0] if (v := shapetracker_getitem(self.st, i))[1] else -1)
            for i in range(prod(self.st.shape))
        ]
...
class TestRealStrides(unittest.TestCase):
    ...
    def test_2(self):
        ...
        st = ShapeTracker(
            (
                View.create(
                    (6, 2, 5, 14), (90, 45, 1, 5), 0, ((0, 6), (0, 2), (0, 5), (0, 9))
                ),
    ...
    def test_3(self):
        ...
        st = ShapeTracker(
            (
                View.create(
                    (4, 256, 512), (256, 0, 1), 0, ((0, 4), (0, 256), (0, 256))
                ),
...
class TestViewMinify(unittest.TestCase):
    def test_minifies(self):
        ...
        assert (
            len(View.create((10, 10, 10, 10)).permute((1, 0, 2, 3)).minify().shape) == 3
        )
...
class TestIndexExpressions2d(unittest.TestCase):
    def setUp(self):
        ...
        self.sts = [
            ShapeTracker.from_shape((prod(base_shape) + offset,))
            .shrink(((offset, offset + prod(base_shape)),))
            .reshape(base_shape)
    ...
    def tearDown(self):
        for st, offset, shape, idxs_expr in zip(
            self.sts, self.offset, self.shapes, self.idxs_exprs
        ):
            numel = prod(shape)
    ...
    def test_splitting_big(self):
        ...
        assert len(self.st.views) == 1
        v = self.st.views[-1]
        assert v.strides == (0, 5, 1, 0, 15) and v.mask == (
            (0, 1),
            (0, 3),
            (0, 5),
            (0, 1),
...
class TestShapeTrackerEquality(unittest.TestCase):
    def test_simple_equals(self):
        self.assertEqual(
            ShapeTracker.from_shape((10, 10)), ShapeTracker.from_shape((10, 10))
        )
...
class TestMaskedShapeTracker(unittest.TestCase):
    ...
    def test_axis_is_masked(self):
        st = ShapeTracker.from_shape((100, 100, 100, 100)).pad(
            ((0, 1), (0, 0), (2, 0), (0, 0))
        )
    ...
    def test_axis_is_masked_rw1(self):
        st = ShapeTracker(
            views=(
                View(
                    shape=(1, 2, 1, 4, 4, 13, 4, 13),
                    strides=(0, 324, 0, 81, 0, 9, 0, 1),
                    offset=-20,
                    mask=(
                        (0, 1),
                        (0, 2),
                ...
                View(
                    shape=(2, 4, 11, 11, 4, 3, 3),
                    strides=(10816, 0, 52, 1, 2704, 728, 14),
                    offset=0,
                    mask=None,
                    contiguous=False,
                ),
...
class TestShapeTrackerSize(unittest.TestCase):
    ...
    def test_size_variable(self):
        st = ShapeTracker(
            views=(
                View(
                    shape=(1, 1, 1, (Variable("start_pos", 0, 8192) + 1), 1, 8, 4, 128),
                    strides=(0, 0, 0, 1024, 0, 128, 0, 1),
                    offset=0,
                    mask=None,
                    contiguous=False,
                ),
                View(
                    shape=(1, 32, 1, (Variable("start_pos", 0, 8192) + 1), 128),
                    strides=(0, 128, 0, 4096, 1),
                    offset=0,
                    mask=None,
                    contiguous=False,
                ),
