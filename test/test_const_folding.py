...
class TestBitcastConstFolding(unittest.TestCase):
    def test_scalar_bitcast(self):
        def t(cases: dict[DType, Any]):
            for (from_dt, from_v), (to_dt, to_v) in itertools.product(
                cases.items(), cases.items()
            ):
                if not math.isnan(from_v):
                    r = full_graph_rewrite(
                        UOp.const(from_dt, from_v).bitcast(to_dt).sink()
                    ).src[0]
                    self.assertEqual(
                        r.op,
                        Ops.CONST,
                        msg := f"{from_dt} -> {to_dt} ({from_v} -> {to_v})",
                    )
        ...
        t(
            {
                dtypes.int32: 1050081145,
                dtypes.uint32: 1050081145,
                dtypes.float32: 0.29485681653022766,
            }
        )
        t(
            {
                dtypes.int64: 4598983288165178391,
                dtypes.uint64: 4598983288165178391,
                dtypes.float64: 0.29485681936461233,
            }
        )

    def test_vec_bitcast(self):
        r = full_graph_rewrite(
            UOp.const(dtypes.int32.vec(3), (-1, -(2**31), 75))
            .bitcast(dtypes.uint32.vec(3))
            .sink()
        ).src[0]
