...
def to_uops_list(u: list[UOp], opts=None, skip_check=False) -> list[UOp]:
    return linearize_uop(full_graph_rewrite(UOp.sink(*u), opts), skip_check)


def _uops_to_prg(uops_list):
    uops = linearize_uop(
        full_graph_rewrite(
            ast := UOp.sink(*uops_list), opts=Device[Device.DEFAULT].renderer
        )
    )
    ...
    return CompiledRunner(
        ProgramSpec(
            ...
            ast,
            uops=uops,
            global_size=[1, 1, 1] if has_local else None,
            local_size=[1, 1, 1] if has_local else None,
        )
    )
...
def _test_single_value(vals, op, dts):
    ...
    buf_loads = [
        uop(uops, Ops.DEFINE_GLOBAL, dtype.ptr(), (), i + 1)
    ...
    loads = (
        uop(
            ...
            Ops.LOAD,
            dtype,
            [buf_loads[i].index(uop(uops, Ops.CONST, dtypes.int32, (), 0))],
        )
        for i, dtype in enumerate(dts)
    )
    ...
    out = uop(
        ...
        Ops.STORE,
        dtypes.void,
        (buf_store.index(uop(uops, Ops.CONST, dtypes.int32, (), 0)), alu),
    )
    buf = Buffer(Device.DEFAULT, 1, output_dtype).allocate()
    buf2 = [
        Buffer(Device.DEFAULT, 1, dtype)
        .allocate()
        .copyin(np.array([a], dtype=_to_np_dtype(dtype)).data)
        for a, dtype in zip(vals, dts)
    ]
...
def _test_uops_result(output_dtype, uops, res):
    ...
    out = uop(
        ...
        Ops.STORE,
        dtypes.void,
        (buf_store.index(uop(uops, Ops.CONST, dtypes.int32, (), 0)), res),
    )
    buf = Buffer(Device.DEFAULT, 1, output_dtype).allocate()
    prg = _uops_to_prg([out])
    prg.exec([buf])
...
class TestUOps(unittest.TestCase):
    ...
    def _test_bop_fxn(
        self, op, fxn, dts=(dtypes.float32,) * 2, no_b_zero=False, no_b_neg=False
    ):
        for f in [_test_single_value, _test_single_value_const]:
            for a in [-2.0, 0.0, 1.0]:
...
class TestFloatUOps(TestUOps):
    ...
    def test_exp2(self):
        self._test_uop_fxn(Ops.EXP2, lambda a: np.exp2(a))
    ...
    @unittest.skipIf(Device.DEFAULT == "CLANG", "not supported as uop")
    def test_sin(self):
        self._test_uop_fxn(Ops.SIN, lambda a: math.sin(a))

    def test_recip(self):
        self._test_uop_fxn(Ops.RECIP, lambda a: 1 / a if a != 0 else float("inf"))

    def test_sqrt(self):
        self._test_uop_fxn(Ops.SQRT, lambda a: math.sqrt(a) if a >= 0 else float("nan"))

    def test_add(self):
        self._test_bop_fxn(Ops.ADD, lambda a, b: a + b)

    def test_mul(self):
        self._test_bop_fxn(Ops.MUL, lambda a, b: a * b)

    def test_max(self):
        self._test_bop_fxn(Ops.MAX, lambda a, b: max(a, b))

    def test_cmplt(self):
        self._test_bop_fxn(Ops.CMPLT, lambda a, b: a < b)

    def test_cmpne(self):
        self._test_bop_fxn(Ops.CMPNE, lambda a, b: a != b)
    ...
    def test_where(self):
        self._test_top_fxn(
            Ops.WHERE,
            lambda a, b, c: b if a != 0 else c,
            (dtypes.bool, dtypes.float, dtypes.float),
        )
    ...
    def test_mulacc(self):
        self._test_top_fxn(
            Ops.MULACC,
            lambda a, b, c: a * b + c,
            (dtypes.float, dtypes.float, dtypes.float),
        )
...
class TestNonFloatUOps(TestUOps):
    def test_add_int32(self):
        self._test_bop_fxn(
            Ops.ADD, lambda a, b: int(a) + int(b), (dtypes.int32, dtypes.int32)
        )

    def test_mul_int32(self):
        self._test_bop_fxn(
            Ops.MUL, lambda a, b: int(a) * int(b), (dtypes.int32, dtypes.int32)
        )

    @unittest.skipUnless(getenv("PTX"), "only ptx uses bitshifts")
    def test_shr_int32(self):
        self._test_bop_fxn(
            Ops.SHR,
    ...
    def test_and_int32(self):
        self._test_bop_fxn(
            Ops.AND, lambda a, b: int(a) & int(b), (dtypes.int32, dtypes.int32)
        )

    def test_or_int32(self):
        self._test_bop_fxn(
            Ops.OR, lambda a, b: int(a) | int(b), (dtypes.int32, dtypes.int32)
        )
    ...
    def test_cmplt_int32(self):
        self._test_bop_fxn(
            Ops.CMPLT, lambda a, b: int(a) < int(b), (dtypes.int32, dtypes.int32)
        )

    def test_cmpne_int32(self):
        self._test_bop_fxn(
            Ops.CMPNE, lambda a, b: int(a) != int(b), (dtypes.int32, dtypes.int32)
        )

    @unittest.skipUnless(is_dtype_supported(dtypes.bool), "dtype not supported")
    def test_mul_bool(self):
        self._test_bop_fxn(
            Ops.MUL, lambda a, b: bool(a) and bool(b), (dtypes.bool, dtypes.bool)
        )
    ...
    def test_where_float16(self):
        self._test_top_fxn(
            Ops.WHERE,
            lambda a, b, c: b if a != 0 else c,
            (dtypes.bool, dtypes.float16, dtypes.float16),
        )
...
class TestBoolUOps(TestUOps):
    ...
    def test_add_bool(self):
        self._test_bop_bool_fxn(Ops.ADD, lambda a, b: a or b)

    def test_mul_bool(self):
        self._test_bop_bool_fxn(Ops.MUL, lambda a, b: a and b)

    def test_xor_bool(self):
        self._test_bop_bool_fxn(Ops.XOR, lambda a, b: a != b)

    def test_and_bool(self):
        self._test_bop_bool_fxn(Ops.AND, lambda a, b: a & b)

    def test_or_bool(self):
        self._test_bop_bool_fxn(Ops.OR, lambda a, b: a | b)

    def test_cmpne_bool(self):
        self._test_bop_bool_fxn(Ops.CMPNE, lambda a, b: a != b)

    def test_cmplt_bool(self):
        self._test_bop_bool_fxn(Ops.CMPLT, lambda a, b: a < b)

    def test_where_bool(self):
        self._test_top_bool_fxn(Ops.WHERE, lambda a, b, c: b if a else c)
...
class TestExecALU(TestUOps):
    ...
    def test_div(self):
        ...
        np.testing.assert_allclose(
            exec_alu(
                Ops.MUL,
                dtypes.float32,
                (7.0, exec_alu(Ops.RECIP, dtypes.float32, (3.0,))),
            ),
            2 + (1.0 / 3.0),
        )
        np.testing.assert_allclose(
    ...
    def test_recip(self):
        ...
        np.testing.assert_allclose(
            exec_alu(Ops.RECIP, dtypes.float32, ((32 + 521 + 3),)), 1 / (32 + 521 + 3)
        )
        np.testing.assert_allclose(
            exec_alu(Ops.RECIP, dtypes.float32, ((34**2),)), 1 / (34**2)
        )
    ...
    def test_bool_where(self):
        ...
        np.testing.assert_allclose(
            exec_alu(Ops.WHERE, dtypes.float, (False, 2.2, 4.5)), 4.5
        )
    ...
    def test_overflow(self):
        ...
        # test no truncate
        self.assertEqual(
            exec_alu(Ops.ADD, dtypes.uint8, (250, 250), truncate_output=False), 500
        )
...
class TestGatedStoreRewrite(unittest.TestCase):
    def test_tiny_gate_store(self):
        ...
        idx = UOp(
            Ops.INDEX, dtypes.float.ptr(), (gmem, gidx0 * UOp.const(dtypes.int, 2))
        )
...
class TestLocalAccess(unittest.TestCase):
    ...
    @unittest.skipUnless(
        Device[Device.DEFAULT].renderer.has_shared, "test requires shared memory"
    )
    def test_local_basic(self):
        uops = []
        smem = uop(
            uops, Ops.DEFINE_LOCAL, dtypes.float32.ptr(size=16, local=True), (), "smem"
        )
        st = uop(
            uops,
            Ops.STORE,
            dtypes.void,
            (
                smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 0)),
                uop(uops, Ops.CONST, dtypes.float32, (), 42.0),
            ),
        )
        ...
        sres = uop(
            ...
            Ops.LOAD,
            dtypes.float32,
            (smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 0)), barr),
        )
        self.assertEqual(_test_uops_result(dtypes.float32, uops, sres), 42)

    # NOTE: webgpu specific, since only webgpu performs bitpacking for uchar
    @unittest.skipUnless(
        Device.DEFAULT == "WEBGPU", "Test local access with packed data type"
    )
    def test_local_packed(self):
        uops = []
        smem = uop(
            uops, Ops.DEFINE_LOCAL, dtypes.uint8.ptr(size=16, local=True), (), "smem"
        )
        st = uop(
            uops,
            Ops.STORE,
            ...
            (
                smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 0)),
                uop(uops, Ops.CONST, dtypes.uint8, (), 42),
            ),
        )
        barr = uop(uops, Ops.BARRIER, dtypes.void, (st,))
        sres = uop(
    ...
    def test_local_indirect(self):
        uops = []
        smem = uop(
            uops, Ops.DEFINE_LOCAL, dtypes.int32.ptr(size=16, local=True), (), "smem"
        )
        st1 = uop(
            uops,
            Ops.STORE,
            dtypes.void,
            (
                smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 1)),
                uop(uops, Ops.CONST, dtypes.int32, (), 2),
            ),
        )
        st2 = uop(
            uops,
            ...
            dtypes.void,
            (
                smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 2)),
                uop(uops, Ops.CONST, dtypes.int32, (), 42),
            ),
        )
        ...
        ofs = uop(
            ...
            Ops.LOAD,
            dtypes.int32,
            (smem.index(uop(uops, Ops.CONST, dtypes.int32, (), 1)), barr),
        )
...
class TestUOpMethod(unittest.TestCase):
    ...
    def test_compare_alu_same_src_different_arg(self):
        ...
        assert (add < mul) or (
            mul < add
        ), "add and mul with same src should have an order"
    ...
    def test_device(self):
        x = UOp(
            Ops.VIEW,
            dtypes.int,
            (UOp.new_buffer(Device.DEFAULT, 1, dtypes.int), UOp.const(dtypes.int, 1)),
...
class TestUOpStr(unittest.TestCase):
    def test_uop_str(self):
        a = UOp(Ops.CONST, dtypes.float, (), 2.0) + UOp(
            Ops.CONST, dtypes.float, (), 3.0
        )
        for _ in range(20):
            a = a + a
        ...
        with Context(NOOPT=1):
            sink = UOp(
                ...
                dtypes.void,
                (
                    get_kernel(Device[Device.DEFAULT].renderer, t.schedule()[-1].ast)
                    .linearize()
                    .uops[-1],
                ),
            )
    ...
    def test_vectorized_str(self):
        vec = UOp(
            Ops.VECTORIZE,
            dtypes.int.vec(4),
            tuple(UOp.const(dtypes.int, x) for x in range(4)),
        )
...
class TestIndexingOrdering(unittest.TestCase):
    ...
    def test_simple_order(self):
        ...
        st0 = UOp(
            Ops.STORE,
            dtypes.float.vec(4),
            (buf, UOp.const(dtypes.int, 0), UOp.const(dtypes.float.vec(4), 42)),
        )
        st1 = UOp(
            Ops.STORE,
            dtypes.float,
            (buf, UOp.const(dtypes.int, 4), UOp.const(dtypes.float, 10)),
        )
        uops = to_uops_list([st1, st0], skip_check=True)
        stores = [st for st in uops if st.op is Ops.STORE]
        assert (
            stores[0].src[1] < stores[1].src[1]
        ), f"stored at idx {stores[1].src[1].arg} AFTER {stores[0].src[1].arg}"
    ...
    def test_ordering_multi_output(self):
        ...
        st0_0 = UOp(
            Ops.STORE,
            dtypes.float.vec(4),
            (buf0, UOp.const(dtypes.int, 0), UOp.const(dtypes.float.vec(4), 42)),
        )
        st1_0 = UOp(
            Ops.STORE,
            dtypes.float,
            (buf0, UOp.const(dtypes.int, 4), UOp.const(dtypes.float, 10)),
        )
        st0_1 = UOp(
            Ops.STORE,
            dtypes.float.vec(4),
            (buf1, UOp.const(dtypes.int, 0), UOp.const(dtypes.float.vec(4), 42)),
        )
        st1_1 = UOp(
            Ops.STORE,
            dtypes.float,
            (buf1, UOp.const(dtypes.int, 4), UOp.const(dtypes.float, 10)),
        )
    ...
    def test_simple_order_with_special(self):
        ...
        st0 = UOp(
            Ops.STORE,
            dtypes.float.vec(4),
            (buf, gidx0 + UOp.const(dtypes.int, 0), UOp.const(dtypes.float.vec(4), 42)),
        )
        st1 = UOp(
            Ops.STORE,
            dtypes.float,
...
class TestUPatHelpers(unittest.TestCase):
    def test_location(self):
        self.assertEqual(
            sym.patterns[-1][0].location[0].replace("\\", "/").split("/")[-1],
            "symbolic.py",
        )
        self.assertEqual(
            fix_kernel_ops.patterns[0][0].location[0].replace("\\", "/").split("/")[-1],
            "schedule.py",
        )
        self.assertEqual(
            spec.patterns[0][0].location[0].replace("\\", "/").split("/")[-1], "ops.py"
        )
        with self.assertRaises(
            AssertionError
        ):  # TODO: location UPat files created in test/*?
            test_upat = UPat(Ops.CONST, dtypes.bool)
            self.assertEqual(
                test_upat.location[0].split("/")[-1],
                __file__.replace("\\", "/").split("/")[-1],
            )
...
class TestUopsObject(unittest.TestCase):
    ...
    def test_timing(self):
        with Timing("create 10k uops:"):
            ret = [UOp(Ops.CONST, dtypes.int, arg=10000000 + i) for i in range(10000)]
...
class TestShapeSpec(unittest.TestCase):
    ...
    def test_expanded_const(self):
        ...
        self.assertEqual(
            a.st, ShapeTracker.from_shape(()).reshape((1, 1)).expand((4, 4))
        )
    ...
    def test_assign_permuted(self):
        ...
        a = buffer.permute((1, 2, 0)).assign(
            Tensor.arange(4).reshape(1, 2, 2).contiguous()
        )
        a.realize()
    ...
    def test_assign_reshaped(self):
        ...
        assign_pattern = UPat(
            Ops.ASSIGN, src=(UPat(Ops.RESHAPE, src=(UPat(Ops.BUFFER))), UPat())
        )
        assert assign_pattern.match(a.lazydata, {})
        a.realize()
        self.assertEqual(buffer.tolist(), [0, 0, 0, 0])
    ...
    def test_setitem(self):
        ...
        self.assertEqual(
            assign.lazydata.src[0].st, ShapeTracker.from_shape((4,)).shrink(((1, 2),))
        )
    ...
    def test_st_wmma_none(self):
        A = UOp(
            Ops.DEFINE_VAR,
            dtypes.float.vec(16),
            arg=("a", UOp.const(dtypes.float, 0), UOp.const(dtypes.float, 1)),
        )
        B = UOp(
            Ops.DEFINE_VAR,
            dtypes.float.vec(16),
            arg=("b", UOp.const(dtypes.float, 0), UOp.const(dtypes.float, 2)),
        )
        C = UOp(
            Ops.DEFINE_VAR,
            dtypes.float.vec(16),
            arg=("c", UOp.const(dtypes.float, 0), UOp.const(dtypes.float, 3)),
        )
