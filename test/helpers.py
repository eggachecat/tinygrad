...
from tinygrad.runtime.ops_python import (
    PythonProgram,
    PythonRenderer,
    PythonCompiler,
    PythonAllocator,
...
def assert_jit_cache_len(fxn, expected_len):
    ...
    if issubclass(type(fxn.jit_cache[0].prg), Runner) and not type(
        fxn.jit_cache[0].prg
    ).__name__.endswith("Graph"):
        assert (
            len(fxn.jit_cache) == expected_len
        ), f"expected {expected_len}, got {len(fxn.jit_cache)}"
    else:
...
def ast_const(
    ...
    val: ConstType,
    shape: tuple[sint, ...] = (),
    st: Optional[ShapeTracker] = None,
    st_src: Optional[tuple[UOp]] = None,
) -> UOp:
    if st_src is None:
        st_src = (
            (
    ...
    if all(v.mask is None for v in st.views):
        return UOp.const(dtype, val).replace(src=(st.to_uop(),))
...
def eval_uop(uop: UOp, inputs: list[tuple[DType, list[Any]]] | None = None):
    ...
    for buf_dt, data in inputs or []:
        ...
        allocator._copyin(
            buf, memoryview(struct.pack(str(len(data)) + buf_dt.fmt, *data))
        )
    g = UOp(Ops.DEFINE_GLOBAL, uop.dtype.ptr(), arg=0, src=())
    rw = full_graph_rewrite(
        UOp.store(g.index(UOp.const(dtypes.int, 0)), uop).sink(), PythonRenderer
    )
    prog = PythonProgram(
        "run", PythonCompiler().compile(PythonRenderer().render(linearize_uop(rw)))
    )
