...
class Kernel:
    ...
    def first_reduce(self) -> int:
        print(
            f"{list(zip(self.sts[0].shape[:self.first_upcast]+(0,), self.full_shape[:self.first_upcast]+(1,)))=}"
        )
        return [
            resolve(x != y)
            for x, y in zip(
    ...
    def apply_opt(self, opt: Opt, append_opt: bool = True):
        ...
        check(
            axis < len(self.full_shape), f"invalid axis {axis} for {self.full_shape=}"
        )
    ...
    def to_program(self, name_override: Optional[str] = None) -> ProgramSpec:
        ...
        return ProgramSpec(
            self.name if not name_override else name_override,
            src,
            self.opts.device,
            self.ast,
            self.uops,
