...
uops_colors = {
    ...
    Ops.CONST: "#e0e0e0",
    Ops.VCONST: "#e0e0e0",
    Ops.DEFINE_GLOBAL: "#ffe0b0",
    Ops.DEFINE_LOCAL: "#ffe0d0",
    Ops.DEFINE_ACC: "#f0ffe0",
...
def to_metadata(k: Any, v: TrackedGraphRewrite) -> GraphRewriteMetadata:
    return {
        ...
        "code_line": lines(v.loc[0])[v.loc[1] - 1].strip(),
...
def get_metadata(
    keys: list[Any], contexts: list[list[TrackedGraphRewrite]]
) -> list[tuple[str, list[GraphRewriteMetadata]]]:
    return [
        (k.name if isinstance(k, Kernel) else str(k), [to_metadata(k, v) for v in vals])
        for k, vals in zip(keys, contexts)
    ]
...
def uop_to_json(x: UOp) -> dict[int, tuple[str, list[int], str]]:
    ...
    for u in (toposort := x.toposort):
        ...
        if u.op in {Ops.DEVICE, Ops.CONST, Ops.UNIQUE}:
            excluded.add(u)
        ...
        if (
            ...
            and all(
                cr.op is Ops.CONST
                for c in u.src[0].children
                if (cr := c()) is not None and cr in toposort
            )
        ):
    ...
    for u in toposort:
        ...
        if u.op is Ops.VIEW:
            argst = "\n".join(
                [
                    ...
                    + ("" if v.offset == 0 else f" / {v.offset}")
                    for v in unwrap(u.st).views
                ]
            )
        ...
        graph[id(u)] = (
            label,
            [id(x) for x in u.src if x not in excluded],
            uops_colors.get(u.op, "#ffffff"),
        )
    return graph


def get_details(
    k: Any, ctx: TrackedGraphRewrite
) -> Generator[GraphRewriteDetails, None, None]:
    yield {
        "graph": (sink_json := uop_to_json(sink := ctx.sink)),
        "uop": str(sink),
        "changed_nodes": None,
        "diff": None,
        "upat": None,
    ...
    for u0, u1, upat in tqdm(ctx.matches):
        ...
        yield {
            "graph": (sink_json := uop_to_json(sink)),
            "uop": str(sink),
            "changed_nodes": [id(x) for x in u1.toposort if id(x) in sink_json],
            "diff": list(
                difflib.unified_diff(
                    pcall(str, u0).splitlines(), pcall(str, u1).splitlines()
                )
            ),
...
def prep_ts(device: str, ts: decimal.Decimal, is_copy):
    return int(decimal.Decimal(ts) + devices[device][is_copy])


def dev_to_pid(device: str, is_copy=False):
    return {"pid": devices[device][2], "tid": int(is_copy)}
    ...
    return [
        {
            "name": ev.name,
            "ph": "X",
            "ts": prep_ts(ev.device, ev.st, ev.is_copy),
            "dur": float(ev.en - ev.st),
            **dev_to_pid(ev.device, ev.is_copy),
        }
    ]
    ...
    for i, e in enumerate(ev.ents):
        ...
        ret += [
            {
                "name": e.name,
                "ph": "X",
                "ts": prep_ts(e.device, st, e.is_copy),
                "dur": float(en - st),
                **dev_to_pid(e.device, e.is_copy),
            }
        ]
        for dep in ev.deps[i]:
            ...
            ret += [
                {
                    "ph": "s",
                    **dev_to_pid(d.device, d.is_copy),
                    "id": reccnt + len(ret),
                    "ts": prep_ts(d.device, ev.sigs[d.en_id], d.is_copy),
                    "bp": "e",
                }
            ]
            ret += [
...
def to_perfetto(profile: list[ProfileEvent]):
    ...
    prof_json = [
        ...
        for x in dev_ev_to_perfetto_json(ev)
    ]
    for ev in tqdm(profile, desc="preparing profile"):
        if isinstance(ev, ProfileRangeEvent):
            prof_json += range_ev_to_perfetto_json(ev)
        elif isinstance(ev, ProfileGraphEvent):
            prof_json += graph_ev_to_perfetto_json(ev, reccnt=len(prof_json))
    return (
        json.dumps({"traceEvents": prof_json}).encode() if len(prof_json) > 0 else None
    )
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if (url := urlparse(self.path)).path == "/":
            ...
            with open(
                os.path.join(os.path.dirname(__file__), "perfetto.html"), "rb"
            ) as f:
                ret = f.read()
            ...
            if "kernel" in (query := parse_qs(url.query)):
                ...
                def getarg(k: str, default=0):
                    return int(query[k][0]) if k in query else default
        ...
        elif url.path == "/get_profile" and perfetto_profile is not None:
            ret, content_type = perfetto_profile, "application/json"
        else:
            status_code = 404
...
if __name__ == "__main__":
    ...
    if len(getenv("BROWSER", "")) > 0:
        webbrowser.open(f"{HOST}:{PORT}{'/profiler' if contexts is None else ''}")
    try:
