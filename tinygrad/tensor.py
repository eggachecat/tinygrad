...
def _apply_map_to_tensors(applied_map: dict[UOp, UOp]) -> None:
    ...
    fixed_tensors: list[Tensor] = [
        ...
        if (t := tref()) is not None and t.lazydata in all_uops
    ]
...
def _metaop(
    ...
    if isinstance(device, str):
        return UOp.metaop(op, shape, dtype, device, arg)
...
def _frompy(x: Union[list, tuple, bytes], dtype: DType) -> UOp:
    if isinstance(x, bytes):
        ret, data = (
            UOp.metaop(Ops.EMPTY, (len(x) // dtype.itemsize,), dtype, "PYTHON"),
            x,
        )
    else:
        ...
        data = struct.pack(
            ...
            *[truncate_function(xi) for xi in fully_flatten(x)],
        )
    # fake realize
    ret.buffer.allocate(
...
class Tensor(SimpleMathTrait):
    ...
    def __init__(
        self,
        data: Union[None, ConstType, bytes, list, tuple, UOp, "np.ndarray", pathlib.Path],  # type: ignore [name-defined] # noqa: F821
        ...
        device = (
            tuple(Device.canonicalize(x) for x in device)
            if isinstance(device, (tuple, list))
            else Device.canonicalize(device)
        )
        ...
        if isinstance(data, UOp):
            ...
            if isinstance(data, UOp) and data.op is Ops.BIND:
                data = _metaop(Ops.BIND, tuple(), dtype or data.dtype, device, data)
        elif data is None:
            data = _metaop(Ops.EMPTY, (0,), dtype or dtypes.default_float, device)
        elif isinstance(data, get_args(ConstType)):
            data = _metaop(
                Ops.CONST, tuple(), dtype or dtypes.from_py(data), device, data
            )
            ...
            if dtype == dtypes.bfloat16:
                data = (
                    Tensor(_frompy(data, dtypes.float32), device=device)
            ...
            if data.shape == ():
                data = _metaop(
                    Ops.CONST,
                    tuple(),
                    dtype or _from_np_dtype(data.dtype),
                    device,
                    data.item(),
                )
            else:
                data = _fromnp(data.astype(npdtype) if dtype is not None and (npdtype := _to_np_dtype(dtype)) is not None else data)  # type: ignore [name-defined]
        elif isinstance(data, pathlib.Path):
            dtype = dtype or dtypes.uint8
            data = _metaop(
                Ops.EMPTY,
                (data.stat().st_size // dtype.itemsize,),
                dtype,
                f"DISK:{data.resolve()}",
            )
        ...
        if isinstance(device, str):
            self.lazydata: UOp = (
                data if data.device == device else data.copy_to_device(device)
            )
        # if device is a tuple, we should have/construct a MultiLazyBuffer
        elif isinstance(data, UOp) and isinstance(data.device, str):
            self.lazydata = Tensor(data).shard(device).lazydata
        else:
            assert (
                data.device == device
            ), f"MultiLazyBuffer device mismatch, {data.device} != {device}"
            self.lazydata = data
    ...
    def __del__(self):
        all_tensors.discard(weakref.ref(self))
    ...
    def _apply_uop(self, fxn: Callable, *x: Tensor, **kwargs) -> Tensor:
        ...
        return Tensor(
            ...
            requires_grad=(
                ...
                else None if None in needs_input_grad else False
            ),
        )

    def _apply_broadcasted_uop(
        self, fxn: Callable, x: Union[Tensor, ConstType], reverse=False
    ) -> Tensor:
    ...
    def schedule_with_vars(
        ...
        if any(x.op is Ops.MULTI for x in big_sink.toposort):
            ...
            big_sink = UOp.sink(
                *flatten(
                    [
                        x.lazydata.src if x.lazydata.op is Ops.MULTI else [x.lazydata]
                        for x in (self,) + lst
                    ]
                )
            )
        ...
        if __debug__:
            type_verify(list(big_sink.toposort), tensor_uop_spec)
    ...
    def assign(self, x) -> Tensor:
        ...
        if isinstance(self.device, str) and self.device.startswith("DISK"):
            if x.__class__ is not Tensor:
                x = Tensor(x, device="CLANG", dtype=self.dtype)
            self.contiguous().realize().lazydata.base.realized.ensure_allocated().copyin(
                x._data()
            )
            return self
        if x.__class__ is not Tensor:
    ...
    def _data(self) -> memoryview:
        ...
        if self.device != "CLANG":
            buf.options = BufferSpec(nolru=True)
    ...
    def data(self) -> memoryview:
        ...
        if TYPE_CHECKING or sys.version_info < (3, 12):
            assert self.dtype.base.fmt != "e"
        return cast(
            memoryview,
            (
                self._data().cast(self.dtype.base.fmt)
                if 0 in self.shape
                else self._data().cast(self.dtype.base.fmt, self.shape)
            ),
        )
    ...
    def numpy(self) -> "np.ndarray":  # type: ignore [name-defined] # noqa: F821
        ...
        if self.dtype.base == dtypes.bfloat16:
            return self.float().numpy()
        assert (
            _to_np_dtype(self.dtype.base) is not None
        ), f"no np dtype for {self.dtype.base}"
        assert all_int(self.shape), f"no data if shape is symbolic, {self.shape=}"
        return np.frombuffer(self._data(), dtype=_to_np_dtype(self.dtype.base)).reshape(
            self.shape
        )
    ...
    def clone(self) -> Tensor:
        ...
        """
        ret = Tensor(
            self.lazydata.clone(), self.device, requires_grad=self.requires_grad
        )
    ...
    def shard(self, devices: tuple[str, ...], axis: Optional[int] = None) -> Tensor:
        ...
        mlb = self.lazydata.shard(
            devices, self._resolve_dim(axis) if axis is not None else None
        )
    ...
    def from_uop(y: UOp, **kwargs) -> Tensor:
        if y.op is Ops.BIND:
            return Tensor(
                y, **kwargs, requires_grad=False
            )  # this is the only UOp allowed in Tensor
        ...
        **kwargs,
        ...
        if isinstance(device, tuple):
            return Tensor(
                UOp.multi(
                    *[
                        UOp.metaop(op, shape, dtype, Device.canonicalize(d), arg)
                        for d in device
                    ],
                    axis=None,
                ),
        ...
        return Tensor(
            UOp.metaop(op, shape, dtype, Device.canonicalize(device), arg),
            device,
            dtype,
            **kwargs,
        )
    ...
    def _threefry_random_bits(key: Tensor, counts0: Tensor, counts1: Tensor):
        ...
        counts0, counts1 = (x & 0xFFFFFFFF).cast(dtypes.uint32), (
            (x >> 32) & 0xFFFFFFFF
        ).cast(dtypes.uint32)
    ...
    def rand(
        ...
        **kwargs,
        ...
        if (numel := prod(shape)) == 0:
            return Tensor.zeros(shape, device=_device, dtype=dtype, **kwargs)
        num = ceildiv(numel * dtype.itemsize, 4)

        # when using MOCKGPU and NV generate rand on CLANG
        if getenv("MOCKGPU") and device.startswith("NV"):
            device = "CLANG"
        ...
        if device not in Tensor._device_seeds:
            ...
            Tensor._device_rng_counters[device] = Tensor(
                [0], device=device, dtype=dtypes.uint32, requires_grad=False
            )
        ...
        counts0 = (
            ...
            + Tensor._device_rng_counters[device]
        )
        ...
        one = Tensor.ones_like(bits, device=bits.device, dtype=dtype).bitcast(
            uint_dtype
        )
    ...
    @staticmethod
    def linspace(
        start: Union[int, float], stop: Union[int, float], steps: int, **kwargs
    ) -> Tensor:
        """
        ...
        if (
            dtype := to_dtype(kwargs.pop("dtype", dtypes.default_float))
        ) == dtypes.bool:
    ...
    def eye(n: int, m: Optional[int] = None, **kwargs) -> Tensor:
        ...
        return (
            x
            if m is None
            else x.pad((None, (0, m - n))) if m > n else x.shrink((None, (0, m)))
        )
    ...
    def rand_like(self, **kwargs) -> Tensor:
        ...
        if isinstance(self.device, tuple):
            ...
            if self.lazydata.axis is None:
                return Tensor.rand(*self.shape, dtype=dtype, **kwargs).shard(
                    self.device
                )
            contiguous = kwargs.pop("contiguous", True)
            sharded_shape = tuple(
                s // len(self.device) if a == self.lazydata.axis else s
            ...
            return Tensor(
                UOp.multi(*rands, axis=self.lazydata.axis),
                device=self.device,
                dtype=dtype,
                **kwargs,
            )
    ...
    def randn(
        ...
        **kwargs,
        ...
        return (
            ...
            .cast(dtype or dtypes.default_float)
    ...
    @staticmethod
    def normal(
        *shape, mean=0.0, std=1.0, requires_grad: Optional[bool] = None, **kwargs
    ) -> Tensor:
        """
    ...
    def gradient(
        ...
        *targets: Tensor,
        ...
        """
        assert (
            gradient is not None or self.shape == tuple()
    ...
    def backward(self, gradient: Optional[Tensor] = None) -> Tensor:
        ...
        tensors_need_grad: list[Tensor] = [
            ...
            and t.lazydata in all_uops
            and t.requires_grad
            and not Tensor.no_grad
        ]
        # clear contexts
        for t, g in zip(
            tensors_need_grad,
            self.gradient(
                *tensors_need_grad, gradient=gradient, materialize_grads=True
            ),
        ):
            assert (
                g.shape == t.shape
    ...
    def reshape(self, shape, *args) -> Tensor:
        ...
        return (
            self._apply_uop(UOp.reshape, arg=new_shape)
    ...
    def pad(
        self,
        padding: Union[Sequence[sint], Sequence[Optional[tuple[sint, sint]]]],
        mode: str = "constant",
        value: float = 0.0,
    ) -> Tensor:
        """
        ...
        # group padding
            ...
            pX = tuple(
                ...
                for p in cast(Sequence[Optional[tuple[sint, sint]]], padding)
            )
        if len(pX) != self.ndim:
        ...
        if mode == "constant":
            ...
            return (
                ...
                if all(resolve(p >= 0) for p in flatten(pX))
                    ...
                    X.shrink(
                        tuple(
                            (-smin(pB, 0), smin(pA + s, s))
                            for (pB, pA), s in zip(pX, X.shape)
                        )
                    ),
                    pads,
                    value,
        ...
        if mode == "circular":
            ...
            orig_shape, X = X.shape, X.repeat(
                tuple(1 + bool(pB) + bool(pA) for pB, pA in pads)
            )
            return X.shrink(
                tuple(
        ...
        for d, (pB, pA) in enumerate(pads):
            ...
            if mode == "replicate":
                (
                    shrB,
                    shrA,
                ) = tuple((0, 1) if i == d else None for i in range(X.ndim)), tuple(
                    (X.shape[i] - 1, X.shape[i]) if i == d else None
                    for i in range(X.ndim)
                )
                xB, xA = (
                    (
                        X.shrink(shr).expand(
                            tuple(p if i == d else None for i in range(X.ndim))
                        )
                        if p > 0
                        else None
                    )
    ...
    def _getitem(self, indices, v: Optional[Tensor] = None) -> Tensor:
            ...
            match index:
                case list() | tuple() | Tensor():
                    if not isinstance(index, Tensor):
                        index = Tensor(index, self.device, requires_grad=False)
                    if not dtypes.is_int(index.dtype):
                        raise IndexError(f"index dtype {index.dtype} is not supported")
                    index = (index.to(self.device) < 0).where(
                        size, 0
                    ) + index  # treat negative index values
                case int() | UOp():  # sint
                    ...
                    boundary = (
                        [index, index + 1]
                        if index >= 0
                        else [index + size, index + size + 1]
                    )
                case slice():
                    if index.step == 0:
                        raise ValueError(f"{index=} cannot have 0 as step")
                    if not all(
                        isinstance(s, int) or s is None
                ...
                case _:
                    raise IndexError(
                        f"{type(index).__name__} indexing is not supported"
                    )
        ...
        if mops := [i for i in indices_parsed if i["index"] is not None]:
            ...
            x = x.shrink(shrinks).flip(
                tuple(i for i, st in enumerate(strides) if st < 0)
            )
            ...
            if any(abs(st) != 1 for st in strides):
                ...
                x = x.pad(
                    tuple((0, round_up(s, st) - s) for s, st in zip(x.shape, strides))
                )
                x = x.reshape(
                    tuple(flatten((s // st, st) for s, st in zip(x.shape, strides)))
                )
                x = x.shrink(
                    tuple(flatten(((0, s), (0, 1)) for s in x.shape[::2]))
                ).reshape(x.shape[::2])
        ...
        x = x.reshape(
            tuple(
                ...
                if not isinstance(index["index"], int)
            )
        )
        ...
        if tops := [
            ...
            reshape_arg = (
                x.shape[: dims[0]] + (1,) * len(big_shape) + x.shape[dims[0] :]
            )
            # sum reduce the extra dims introduced in create masks
            x = (x.reshape(reshape_arg) * mask).sum(
                sum_axis := tuple(d + len(big_shape) for d in dims), acc_dtype=x.dtype
            )
            ...
            if (
                ...
                x = x.permute(
                    ...
                    *range(0, dims[0]),
                    *range(dims[0] + len(big_shape), x.ndim),
                )

            # for advanced setitem, returns whole tensor with indices replaced
            if v is not None:
                vb = v.cast(self.dtype)._broadcast_to(
                    _broadcast_shape(x.shape, v.shape)
                )
                ...
                x = _masked_setitem(
                    self, vb, mask, tuple(range(dims[0], dims[0] + len(big_shape)))
                )
    ...
    def __setitem__(self, indices, v: Union[Tensor, ConstType]) -> None:
        ...
        if not unwrap(self.lazydata.st).contiguous:
    ...
    def gather(self: Tensor, dim: int, index: Tensor) -> Tensor:
        ...
        return (x * index.unsqueeze(-1)._one_hot_along_dim(self.shape[dim])).sum(
            -1, acc_dtype=self.dtype
        )
    ...
    def cat(self: Tensor, *args: Tensor, dim: int = 0) -> Tensor:
        ...
        for i, t in enumerate(tensors):
            tensors[i] = t.pad(
                [
                    (
                        (dim_cumsum[i], dim_cumsum[-1] - dim_cumsum[i + 1])
                        if j == dim
                        else None
                    )
    ...
    def repeat(self, repeats, *args) -> Tensor:
        ...
        return (
            self.reshape(unsqueezed_shape).expand(expanded_shape).reshape(final_shape)
        )
    ...
    def split(self, sizes: Union[int, list[int]], dim: int = 0) -> tuple[Tensor, ...]:
        ...
        assert (
            sum(sizes) == self.shape[dim]
        ), f"expect sizes to sum exactly to {self.shape[dim]}, but got {sum(sizes)}"
        return tuple(
            self[sl]
            for sl in [
                tuple(
                    [slice(None)] * dim + [slice(sum(sizes[:i]), sum(sizes[: i + 1]))]
                )
    ...
    def roll(
        ...
        for dim, shift in zip(dims, make_tuple(shifts, 1)):
            ...
            rolled = Tensor.cat(
                rolled[
                    tuple(
                        slice(None) if i != dim else slice(-shift, None)
                        for i in range(rolled.ndim)
                    )
                ],
                rolled[
                    tuple(
                        slice(None) if i != dim else slice(None, -shift)
                        for i in range(rolled.ndim)
                    )
                ],
                dim=dim,
    ...
    def rearrange(self, formula: str, **sizes) -> Tensor:
        ...
        assert sorted(lhs) == sorted(rhs) and len(lhs) == len(
            set(lhs)
        ...
        assert "..." not in flatten(
            [lhs[s:e] for s, e in unflatten_dims]
        ...
        if "..." in lhs:
            ell_len = (
                ...
                - len(lhs)
                + 1
                + sum(e - s - 1 for s, e in unflatten_dims)
            )
        lhs, rhs = map(
            lambda l: (
                l[: (i := l.index("..."))]
        ...
        for i, name in enumerate(lhs):
            assert (name not in sizes) or sizes[name] == t.shape[
                i
            ], f"size provided for dimension {name} incorrect"
        t = t.permute([lhs.index(name) for name in rhs])
        return functools.reduce(
            lambda x, dims: (
                x.flatten(dims[0], dims[1] - 1)
                if dims[0] < dims[1]
    ...
    def _reduce(
        self, op: Ops, axis: Optional[Union[int, Sequence[int]]] = None, keepdim=False
    ) -> Tensor:
        axis = tuple(
            self._resolve_dim(x)
    ...
    def sum(
        ...
        """
        ret = self.cast(
            sum_acc_dtype(self.dtype) if acc_dtype is None else acc_dtype
        )._reduce(Ops.ADD, axis, keepdim)
        return (
            ret.cast(self.dtype)
            if acc_dtype is None and self.dtype in (dtypes.float16, dtypes.bfloat16)
            else ret
        )
    ...
    def _inverse(self):
        return (
            ...
            else ~self if dtypes.is_int(self.dtype) else self.logical_not()
        )
    ...
    def isclose(
        ...
        def isfinite(t):
            return (t.isinf() | t.isnan()).logical_not()
        ...
        return numerator.div(
            prod(
                [
                    si
                    for si, so in zip(
                        self.shape, self.sum(axis=axis, keepdim=True).shape
                    )
                    if resolve(si != so)
                ]
            )
    ...
    def var(
        ...
        n = prod(
            [
                si
                for si, so in zip(
                    self.shape, squares.sum(axis=axis, keepdim=True).shape
                )
                if resolve(si != so)
            ]
        )
        ...
        *operands: Tensor | Sequence[Tensor],
        ...
        def parse_formula(formula: str, *operands: Tensor):
            if "..." in (formula := formula.replace(" ", "")):
                ell_chars, ell_longest = (
                    "".join(set(string.ascii_letters) - set(formula)),
                    0,
                )
                for i, inp in enumerate(
                    filter(
                        lambda x: "..." in x,
                        inputs := formula.split("->")[0].split(","),
                    )
                ):
                    if (
                    ...
                    if "->" in formula
                        ...
                        + "".join(
                            sorted(
                                c
                                for c in inputs_str
                                if inputs_str.count(c) == 1
                                and c.isalpha()
                                and c not in out_ellipse
                            )
                        ),
                    )
                )
            return (
                ...
                if "->" in formula
                    ...
                    "".join(
                        ...
                        if formula.count(c) == 1 and c.isalpha()
                    ),
                )
            )
        ...
        for x, (order, letters) in zip(xs, [list(zip(*l)) for l in lhs]):
            ...
            xs_.append(
                x.permute(order)
                .reshape(
                    [val if letter in letters else 1 for letter, val in letter_val]
                )
        ...
        return (
            ...
            .sum(
                axis=[
                    axis
                    for axis, (letter, _) in enumerate(letter_val)
    ...
    def _pool(
        ...
        if any(resolve(k > s) for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
            ...
            x = x.shrink(
                tuple(
                    noop + [(0, k * (i * f + d)) for k, i, d, f in zip(k_, i_, d_, f_)]
                )
            ).reshape(
            ...
            x = x.shrink(
                tuple(
                    noop + flatten(((0, k), (0, o * s)) for k, o, s in zip(k_, o_, s_))
                )
    ...
    def _resolve_pool_pads(
        self, padding: Union[int, Sequence[int]], dims: int
    ) -> Sequence[int]:
    ...
    def _apply_ceil_mode(
        ...
        pads: Sequence[int],
        k_: tuple[sint, ...],
        s_: Union[tuple[int, ...], int],
        d_: Union[tuple[int, ...], int],
    ) -> list[int]:
        (d_, s_), i_ = (make_tuple(x, len(k_)) for x in (d_, s_)), self.shape[
            -len(k_) :
        ]
    ...
    def max_pool2d(
        self, kernel_size=(2, 2), stride=None, dilation=1, padding=0, ceil_mode=False
    ):
        """
        ...
        return (
            self.pad(pads, value=dtypes.min(self.dtype))
            ._pool(k_, stride if stride is not None else k_, dilation)
            .max(tuple(range(-len(k_), 0)))
        )
    ...
    def conv2d(
        ...
        if IMAGE:
            return self.image_conv2d(
                weight, bias, groups, stride, dilation, padding, acc_dtype
            )
        ...
        ret = _apply_winograd_matrix(
            winograd_At,
            (gfactors * dfactors).sum(axis=-1 - len(HW), acc_dtype=acc_dtype),
            len(HW),
        )
    ...
    def conv_transpose2d(
        ...
        ```
        """
        x, w = self, weight.unflatten(0, (groups, -1)).transpose(1, 2).flip(
            *range(3, len(weight.shape) + 1)
        )
        HW = weight.shape[2:]
        ...
        if any(s > 1 for s in stride):
            ...
            x = x.shrink(
                (None, None, *[(0, k - (s - 1)) for k, s in zip(x.shape[2:], stride)])
            )
        padding = flatten(
            (
    ...
    def dot(self, w: Tensor, acc_dtype: Optional[DTypeLike] = None) -> Tensor:
        ...
        """
        if IMAGE:
            return self.image_dot(w, acc_dtype)
        ...
        return (
            ...
            .cast(
                least_upper_dtype(x.dtype, w.dtype) if acc_dtype is None else acc_dtype
            )
        )

    def matmul(
        self, x: Tensor, reverse=False, acc_dtype: Optional[DTypeLike] = None
    ) -> Tensor:
        """
        ...
        """
        return (
            x.dot(self, acc_dtype=acc_dtype)
            if reverse
            else self.dot(x, acc_dtype=acc_dtype)
        )
    ...
    def _cumalu(self, axis: int, op: Ops, _include_initial=False) -> Tensor:
        ...
        pooled = (
            ...
            .pad(
                (pl_sz, -int(_include_initial)), value=identity_element(op, self.dtype)
            )
    ...
    def interpolate(
        ...
        for i in range(-1, -len(size) - 1, -1):
            ...
            if mode == "linear":
                index = (
                    scale * arr if align_corners else (scale * (arr + 0.5)) - 0.5
                ).clip(0, self.shape[i] - 1)
                low, high, perc = [
    ...
    def _pre_scatter(
        ...
        assert (
            index.ndim == self.ndim == src.ndim
        ), f"self.ndim, index.ndim and src.dim must all equal, {self.ndim=} {index.ndim=} {src.ndim=}"
        assert all(
        ...
        mask = (
            index.unsqueeze(-1)._one_hot_along_dim(self.shape[dim]).transpose(-1, dim)
        )
    ...
    def scatter(
        ...
        reduce: Union[None, Literal["multiply"], Literal["add"]] = None,
    ) -> Tensor:
        """
    ...
    def scatter_reduce(
        ...
        def _inv_mask(
            a: Union[Tensor, ConstType], b: Union[Tensor, ConstType]
        ) -> Tensor:
        ...
        if reduce == "sum":
            return (
                mask.where(src, 0)
                .sum(-1, acc_dtype=self.dtype)
                .add(self if include_self else _inv_mask(self, 0))
            )
        if reduce == "prod":
            return (
                mask.where(src, 1)
                .prod(-1, acc_dtype=self.dtype)
                .mul(self if include_self else _inv_mask(self, 1))
            )
        if reduce == "amax":
            return (
        ...
        if reduce == "mean":
            count = (
                mask.where(1, 0)
                .sum(-1, acc_dtype=self.dtype)
                .add(1 if include_self else _inv_mask(1, 0))
            )
            return (
                mask.where(src, 0)
                .sum(-1, acc_dtype=self.dtype)
    ...
    def round(self: Tensor) -> Tensor:
        ...
        """
        return (
            (self > 0) == ((b := self.cast(dtypes.int32) / 2.0).cast(dtypes.int32) == b)
        ).where((self - 0.5).ceil(), (self + 0.5).floor())
    ...
    def sign(self):
        ...
        return (
            self.ne(0).where(
                (self < 0).where(self.full_like(-1), self.full_like(1)),
                self.full_like(0),
            )
            + self * 0
        )
    ...
    def mod(self, x: Union[Tensor, ConstType], reverse=False) -> Tensor:
        ...
        """
        a, b = self._broadcasted(x, reverse)
        return (r := a._apply_uop(UOp.mod, b)) + b * (
            ((r < 0) & (b > 0)) | ((r > 0) & (b < 0))
        )
    ...
    def lshift(self, x: int):
        ...
        """
        assert (
            dtypes.is_unsigned(self.dtype) and isinstance(x, int) and x >= 0
    ...
    def __ifloordiv__(self, x) -> Tensor:
        return self.assign(self.idiv(x))
    ...
    def layernorm(
        self, axis: Union[int, tuple[int, ...]] = -1, eps: float = 1e-5
    ) -> Tensor:
        """
    ...
    def batchnorm(
        ...
        ret = x.mul(
            invstd.reshape(shape) if len(invstd.shape) == len(axis_) else invstd
        )
    ...
    def _one_hot_along_dim(self: Tensor, num_classes: sint, dim: int = -1):
        ...
        return self == Tensor.arange(
            num_classes, device=self.device, requires_grad=False
    ...
    def binary_crossentropy_logits(
        ...
        """
        return (
            self.maximum(0) - Y * self + (1 + self.abs().neg().exp()).log()
    ...
    def sparse_categorical_crossentropy(
        ...
        log_probs, loss_mask = self.log_softmax(), (
            (Y != ignore_index)
            if ignore_index != -1
        ...
        y_counted = (
            ...
            .flatten()
            .reshape(-1, 1)
            ._one_hot_along_dim(self.shape[-1])
        )
        ...
        if (
            ...
            if ns > os:
                return functools.reduce(
                    Tensor.add,
                    (
                        tmp[..., i :: ns // os].cast(new_uint) << 8 * i * os
                        for i in range(ns // os)
                    ),
                ).bitcast(dtype)
            return (
    ...
    def image_dot(self, w: Tensor, acc_dtype: Optional[DTypeLike] = None) -> Tensor:
        ...
        cx = self.transpose(self.ndim - 1, self.ndim - 2).reshape(
            (bs // groups, groups * cin, -1, 1)
        )
        # groups*cout x cin x H, W
        cw = w.transpose(w.ndim - 1, w.ndim - 2).reshape((groups * cout, cin, 1, 1))
        return (
            cx.image_conv2d(cw, groups=groups, acc_dtype=acc_dtype)
            .reshape(out_shape_t)
    ...
    def image_conv2d(
        ...
        if cin % 4 != 0 and not (cin == 1 and groups % 4 == 0):
            ...
            w = w.pad(
                tuple(
                    (0, added_input_channels) if i == 2 else None for i in range(w.ndim)
                )
            )
            x = x.pad(
                tuple(
                    (0, added_input_channels) if i == 2 else None for i in range(x.ndim)
                )
            )
        ...
        if rcout % 4 != 0 and not (rcout == 1 and groups % 4 == 0):
            ...
            w = w.pad(
                tuple(
                    (0, added_output_channels) if i == 1 else None
        ...
        if IMAGE >= 2:
            x, w = x.cast(
                base_image_type((bs * iy, ix * groups * cin // 4, 4))
        ...
        w = w.permute(0, 4, 2, 5, 1, 3).reshape(
            (1, 1, 1, *cout_expand, rcin_hi, rcin_lo, H, W)
        )
        ...
        ret = (
            ...
            .sum((-4, -3, -2, -1), acc_dtype=acc_dtype)
        )
...
def _metadata_wrapper(fn):
    def _wrapper(*args, **kwargs):
        ...
        if TRACEMETA >= 2:
            ...
            while (
                caller_module.startswith("tinygrad.nn") and "optim" not in caller_module
            ):
