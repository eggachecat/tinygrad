    ...
    [
        ...
        (
            ...
            + (
                ...
                if ctx.device in {"CLANG", "DSP"}
                else f"({','.join([ctx[y] for y in x.src])})"
            ),
        ),
        (
        ...
        (
            ...
            lambda ctx, x: f"(*(({ctx.buffer_prefix}{ctx.render_dtype(x.dtype)}*)&{ctx[x.src[0]]}))",
        ),
        (
            UPat(Ops.DEFINE_LOCAL, name="x"),
            lambda ctx, x: f"{ctx.smem_align}{ctx.smem_prefix}{ctx.render_dtype(x.dtype.base)} {ctx[x]}[{x.dtype.size}];",
        ),
        (UPat(Ops.BARRIER), lambda ctx: ctx.barrier),
        (UPat(Ops.NOOP, name="x"), lambda ctx, x: ctx[x.src[0]]),
        (
            UPat(Ops.SPECIAL, name="x"),
            lambda ctx, x: f"{ctx.code_for_workitem[x.arg[0][0]](x.arg[0][-1])}; /* {x.arg[1]} */",
        ),
        # const
        (
        ...
        (
            ...
            lambda ctx, x: (
                f"({ctx.render_cast(x.dtype, ctx.nan)})" if math.isnan(x.arg) else None
            ),
        ),
        ...
        (
            UPat(Ops.CONST, (dtypes.bfloat16, dtypes.half), name="x"),
        ...
        (
            ...
            lambda ctx, x: f"({ctx.render_cast(x.dtype, x.arg)})",
        ),
        ...
        (
            UPat(
                Ops.STORE, src=(UPat.var("bidx"), UPat.var("var")), allow_any_len=True
            ),
            ...
            lambda ctx, x: ctx.code_for_op[x.op](
                *(
                    [
                        (
                            strip_parens(ctx[v])
                            if v.op == x.op and x.op in {Ops.ADD, Ops.MUL, Ops.XOR}
                            else ctx[v]
                        )
                        for v in x.src
                    ]
        ...
        (
            ...
            + (
                f"[{x.arg[0]}]"
                if x.src[0].dtype.count > (8 if ctx.device in {"CUDA", "NV"} else 4)
                or ctx.device in {"CLANG", "DSP"}
...
extra_pm = PatternMatcher(
    [
        ...
        (
            UPat(Ops.BITCAST, name="x"),
            lambda x: (
                UOp(Ops.BITCAST, x.dtype, (UOp(Ops.NOOP, x.src[0].dtype, x.src),))
                if x.src[0].op is not Ops.NOOP
                else None
            ),
        ),
        ...
        (
            UPat(
                (*GroupOp.ALU, Ops.CAST, Ops.BITCAST, Ops.ASSIGN, Ops.INDEX),
                dtype=dtypes.bool,
                name="alu",
            ),
            no_vectorized_alu,
...
class CStyleLanguage(Renderer):
    ...
    code_for_workitem: dict[
        Union[Literal["g"], Literal["l"], Literal["i"]], Callable
    ] = {}
    extra_args: list[str] = []
    ...
    code_for_op: dict = {
        ...
        Ops.RECIP: lambda x, dtype: f"(1/{x})",
    ...
    def render_kernel(
        ...
        buftypes = [
            (
                ...
                (
                    ...
                    else self.arg_int_prefix if dtype == dtypes.int else None
                ),
            )
        ...
        prg = "".join(
            [
                f"{self.kernel_prefix}void {self.get_kernel_modifier(uops)}{function_name}(",
            ]
    ...
    def render_dtype(self, dt: DType, mutable=True) -> str:
        ...
        if isinstance(dt, PtrDType):
            return (
                (
                    self.smem_prefix
                    if dt.local and self.smem_prefix_for_cast
                    else self.buffer_prefix
                )
                + self.render_dtype(dt.base)
                + "*"
            )
        if dt.count > 1:
    ...
    def render(self, uops: list[UOp]) -> str:
        ...
        for u in uops:
            ...
            if u.op in {Ops.CONST, Ops.GEP, Ops.INDEX, Ops.CUSTOM} or (
                u.op in {Ops.VECTORIZE, *GroupOp.ALU, Ops.CAST, Ops.BITCAST}
                and child_count[u] == 1
                ...
                if (
                    u.op in {Ops.RANGE, Ops.ASSIGN, Ops.DEFINE_LOCAL}
...
class ClangRenderer(CStyleLanguage):
        ...
        **(
            {
                ...
                if k not in [Ops.EXP2, Ops.SIN, Ops.LOG2]
            }
        ),
        Ops.SQRT: lambda x, dtype: (
            f"__builtin_sqrt({x})"
    ...
    def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
        ...
        for name, (N, M, _), dtype_in, _, _, _, _, _ in dedup(
            [uop.arg for uop in uops if uop.op is Ops.WMMA]
        ):
            prefix += [
            ...
            prefix += [
                f"""static {(out := self.render_dtype(dtype_in.vec(N*N)))} __{name}({self.render_dtype(dtype_in.vec(N))} data1, {self.render_dtype(dtype_in.vec(M))} data2, {out} data0){{
            ...
            [
                ...
                (
                    UPat(
                        ...
                        src=(
                            UPat.var("buf").index(UPat.var("idx", dtypes.int.vec(2))),
                            UPat.var("var"),
                            UPat.var("gate"),
                        ),
                    ),
...
class IntelRenderer(OpenCLRenderer):
            ...
            [
                (
                    UPat(
                        ...
                        src=(UPat.var("x", dtype=dtypes.float)),
                    ),
                ...
                (
                    UPat(
                        Ops.CAST,
                        dtype=dtypes.float,
                        src=(UPat.var("x", dtype=dtypes.bfloat16)),
                    ),
    ...
    def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
        ...
        for arg in dedup([uop.arg for uop in uops if uop.op is Ops.WMMA]):
            dt_in = (
                ("ushort", "bf16")
                if arg[2] == dtypes.bfloat16
                else (arg[2].name, "f16")
            )
            prefix.append(
                f"""{arg[3].name}8 __{arg[0]}({dt_in[0]}16 a, {dt_in[0]}16 b, {arg[3].name}8 c) {{
                ...
                (
                    ...
                    lambda x: (
                        UOp(
                            x.op,
                            dtypes.float,
                            tuple(vv.cast(dtypes.float) for vv in x.src),
                            x.arg,
...
class CUDARenderer(CStyleLanguage):
    ...
    tc_81616 = [
        TensorCore(
            ...
            opts=cuda_tc_opts,
            swizzle=(
                ((6, 7, 2, 3, 4), (0, 1, 9, 5, 10, 8)),
                ((6, 7, 9, 0, 1), (2, 3, 4, 10, 5, 8)),
            ),
        )
        for di, do in [
            (dtypes.half, dtypes.float),
    ...
    code_for_op = {
        **CStyleLanguage.code_for_op,
        Ops.SIN: lambda x, dtype: (
            f"hsin({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"sin({x})"
        ),
        Ops.LOG2: lambda x, dtype: (
            f"hlog2({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"log2({x})"
        ),
        Ops.EXP2: lambda x, dtype: (
            f"hexp2({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"exp2({x})"
        ),
        Ops.SQRT: lambda x, dtype: (
            f"hsqrt({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"sqrt({x})"
        ),
        Ops.RECIP: lambda x, dtype: (
            f"hrcp({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"(1/{x})"
        ),
    }
    ...
    def render_vector_prefix(self, dt: DType) -> str:
        ...
        elems, header = ", ".join(_nms[: dt.count]), ", ".join(
            [f"{scal} {x}" for x in _nms[: dt.count]]
        )
    ...
    def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
        ...
        for name, (N, M, K), dtype_in, dtype_out, _, _, upcast_axes, _ in dedup(
            [uop.arg for uop in uops if uop.op is Ops.WMMA]
        ):
            ...
            prefix.append(
                f"""__device__ {wmma_dtypes[2]} __{name}({wmma_dtypes[0]} a, {wmma_dtypes[1]} b, {wmma_dtypes[2]} c){{
  ...
  return c;\n}}"""
    ...
    def get_kernel_modifier(self, uops: list[UOp]) -> str:
        maxThreadsPerBlock = prod(
            u.arg[1] for u in uops if u.op is Ops.SPECIAL and u.arg[0][0] == "l"
        )
...
class AMDRenderer(CStyleLanguage):
    ...
    tensor_cores = [
        TensorCore(
            ...
            swizzle=(
                ((4, 9, 10, 11, 0), (1, 2, 3, 5, 6, 7, 8)),
                ((0, 1, 2, 3, 4), (9, 10, 11, 5, 6, 7, 8)),
            ),
        )
        for di, do in [(dtypes.half, dtypes.float), (dtypes.half, dtypes.half)]
    ]
            ...
            [
                ...
                (
                    UPat(
                        ...
                        src=(
                            UPat.var("b"),
                            UPat.var("x", dtype=dtypes.bfloat16),
                            UPat.var("y", dtype=dtypes.bfloat16),
                        ),
                    ),
                    lambda b, x, y: UOp(
                        Ops.WHERE,
                ...
                (
                    ...
                    lambda alu, x, y: UOp(
                        alu.op,
                        dtypes.bool,
                        (x.cast(dtypes.float), y.cast(dtypes.float)),
                        alu.arg,
                    ),
                ),
                ...
                (
                    ...
                    lambda x, y: (
                        y.cast(dtypes.float).cast(x.dtype)
                ...
                (
                    ...
                    lambda x: (
                        x.cast(dtypes.float).cast(dtypes.bfloat16)
                        if x.dtype != dtypes.float
                        else None
                    ),
                ),
                ...
                (
                    ...
                    lambda x: (
                        x.bitcast(dtypes.ushort).cast(dtypes.uint) << 16
                    ).bitcast(dtypes.float),
                ),
                (
                    UPat(
                        Ops.CAST,
    ...
    def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
        ...
        for arg in dedup(
            ...
            if arg[3] == dtypes.float:
                prefix.append(
                    f"#define __{arg[0]} __builtin_amdgcn_wmma_f32_16x16x16_f16_w32"
                )
            else:
                prefix.append(
                    f"static inline __attribute__((device)) half8 __{arg[0]}"
  ...
  for (int n = 0; n < 8; n++) { d[n] = c_frag[n*2]; } return d;\n}"""
    ...
    def get_kernel_modifier(self, uops: list[UOp]) -> str:
        requiredMaxThreadsPerBlock = prod(
            u.arg[1] for u in uops if u.op is Ops.SPECIAL and u.arg[0][0] == "l"
        )
