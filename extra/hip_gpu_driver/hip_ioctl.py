...
def ioctls_from_header():
    hdr = (
        (pathlib.Path(__file__).parent / "kfd_ioctl.h").read_text().replace("\\\n", "")
    )
    pattern = r"#define\s+(AMDKFD_IOC_[A-Z0-9_]+)\s+AMDKFD_IOW?R?\((0x[0-9a-fA-F]+),\s+struct\s([A-Za-z0-9_]+)\)"
    matches = re.findall(pattern, hdr, re.MULTILINE)
    return {
        int(nr, 0x10): (name, getattr(kfd_ioctl, "struct_" + sname))
...
def ioctl(fd, request, argp):
    ...
    if nr in nrs and itype == 75:
        ...
        print(
            f"{(st-start)*1000:7.2f} ms +{et*1000.:7.2f} ms : {ret:2d} = {name:40s}",
            " ".join(format_struct(s)),
        )
        if name == "AMDKFD_IOC_SVM":
            out = ctypes.cast(
                s.attrs, ctypes.POINTER(kfd_ioctl.struct_kfd_ioctl_svm_attribute)
            )
            for i in range(s.nattr):
                print(
                    f"{i}: {kfd_ioctl.kfd_ioctl_svm_attr_type__enumvalues[out[i].type]:40s}: {out[i].value:#x}"
                )
    else:
        print(
            f"{(st-start)*1000:7.2f} ms +{et*1000.:7.2f} ms : ioctl",
            f"{idir=} {size=} {itype=} {nr=} {fd=} {ret=}",
            os.readlink(f"/proc/self/fd/{fd}") if fd >= 0 else "",
        )
    return ret
...
if __name__ == "__main__":
    ...
    a = Tensor([1.0, 2.0] * 1024 * 1024, device="HIP").realize()
    print("***** create tensor b")
    b = Tensor([3.0, 4.0] * 1024 * 1024, device="HIP").realize()
